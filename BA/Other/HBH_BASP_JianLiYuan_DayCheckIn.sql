...
from (	
	...
	from
		...
		select 
			...
			,LaborYieldTarget = (select max(IsNull(checkin2.LaborYieldTarget,@DefaultZero)) from Cust_DayCheckIn checkin2 
			...
			,LaborCostTarget = (select max(IsNull(checkin2.LaborCostTarget,@DefaultZero)) from Cust_DayCheckIn checkin2 
								where  checkin.Department = checkin2.Department
									-- 年月相同
									and DatePart(year,checkin.CheckInDate)*100 + DatePart(month,checkin.CheckInDate)
										= DatePart(year,checkin2.CheckInDate)*100 + DatePart(month,checkin2.CheckInDate)
		...
		from 		
			...
			left join CBO_Department region2
			on SubString(dept.Code,1,7) = region2.Code
			left join CBO_Department_Trl region2Trl
