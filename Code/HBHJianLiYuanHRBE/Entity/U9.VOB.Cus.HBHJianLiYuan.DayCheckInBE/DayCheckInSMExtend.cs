...
namespace U9.VOB.Cus.HBHJianLiYuan
{
    ...
    {
        public partial class DayCheckInStateMachine
        {
            ...
            private State Approving_ApproveEventImp(U9.VOB.Cus.HBHJianLiYuan.ApproveEvent ev)
            {
 			...
 			//RefusedDriven事件驱动方法的实现，开发人员在此实现RefusedDriven的逻辑
            private State Approving_RefusedDrivenImp(U9.VOB.Cus.HBHJianLiYuan.RefusedEvent ev)
            {
                // To do ...
				State toState=CurrentState;
				//TODO:实现转移条件
    ...
    [Persistent]
    [Serializable]
    internal class ServiceOrderSubscriber_DayCheckIn : IEventSubscriber
    {
        ...
        public void Notify(params object[] args)
        {
            ...
            if (args.Length == 1)
            {
                ...
                {
                    DayCheckIn entity = this.EntityKey.GetEntity() as DayCheckIn;
        ...
        //private void SetLineStatus(PR pr, ActivityTypeEnum activityType)
        //{
        //    foreach (PRLine current in pr.PRLineList)
        //    {
